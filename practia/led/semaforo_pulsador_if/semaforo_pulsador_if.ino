////a ver 2 intento con el else funciono
...
void setup(){
  ...
  pinMode(zumba, OUTPUT);    //salida sonido
...
void cambioLuz() {
  ...
  digitalWrite(pverde, HIGH);  // verde encendido, peatón
    if (pverde == HIGH){           // bucle que se repite 5 veces
       digitalWrite(zumba, HIGH);    // sonido
       delay(200);                      // pausa
       digitalWrite(zumba, LOW);     // silencio 
       delay(1000);// pausa
  }else{
    digitalWrite(zumba, HIGH);    // sonido aviso de cruce
                          
    
   }
  delay(tiempoCruce);
  digitalWrite(zumba, LOW);     // silencio no se cruza
